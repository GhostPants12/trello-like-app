...
import {
    Card
} from '../cards/card.entity';
import {
    UserBoard
} from '../boards/userBoard.entity';
import {
    User
...
export class CommentCreatorOrAdminGuard implements CanActivate {
    ...
    async canActivate(context: ExecutionContext): Promise < boolean > {
        ...
        const comment = await this.commentRepository.findByPk(commentId, {include:User});
        if (comment !== null) {
            if (comment.author.id === userId) {
                ...
                if (card !== null) {
                    return (await this.userBoardRepository.findOne({
                        where: {
