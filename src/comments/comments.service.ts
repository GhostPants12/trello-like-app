...
export class CommentsService {
  ...
  async createComment(userId: number, cardId: number, comment: CommentDto) {
    ...
    commentRecord = await this.commentRepository.findByPk(commentRecord.id, {
    ...
    for (const user of mentions) {
      ...
      if (
        userRecord !== null &&
        commentRecord.card.list.board.users.some(
          (user) => user.id == userRecord.id,
        )
      ) {
        await this.mailerService.sendMail({
          ...
          text: JSON.stringify({
            ...
            board: commentRecord.card.list.board.name,
            card: commentRecord.card.name,
            text: commentRecord.text,
          }),
          html: `<p>User <b>${commentRecord.author.username}</b> mentioned you in a card <b>${commentRecord.card.name}</b> in a board <b>${commentRecord.card.list.board.name}</b>.
  ...
  async updateComment(commentId, comment: CommentDto) {
    ...
    const commentRecord = await this.commentRepository.findByPk(commentId, {
      include: { all: true, nested: true },
    });
    const mentionRe = /@[a-z0-9]+/i;
    const mentions = comment.text.match(mentionRe);
